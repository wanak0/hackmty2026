    ...
    return (
      <div className="p-4 rounded-2xl border border-gray-200 bg-white shadow-xs space-y-3">
        ...
        <div className="flex items-end gap-3 h-36 pt-2">
          {bars.map((bar, idx) => {
            const h = Math.max(8, Math.round(((Number(bar.value) || 0) / max) * 100));
            return (
              <div key={idx} className="flex-1 flex flex-col items-center gap-1.5 min-w-0">
                <span className="text-[10px] font-bold text-gray-600 truncate w-full text-center">
                  ${Number(bar.value).toLocaleString('es-MX')}
                </span>
        ...
        {unit && <p className="text-[10px] text-gray-400 font-medium">Unidad: {unit}</p>}
      </div>
    );
  }
  ...
  return (
    <div className="p-4 rounded-2xl border border-gray-200 bg-white shadow-xs space-y-3">
      ...
      <div className="space-y-3">
        {bars.map((bar, idx) => {
          const w = Math.max(4, Math.round(((Number(bar.value) || 0) / max) * 100));
          return (
            <div key={idx} className="space-y-1">
              <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-1.5 min-w-0">
                  {bar.icon && <Icon name={bar.icon} tone="muted" size="sm" />}
                  <span className="text-xs font-bold text-gray-700 truncate">{bar.label}</span>
                </div>
