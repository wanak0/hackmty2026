      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-40 h-40 shrink-0">
            ...
            {segments.map((seg, idx) => {
              ...
              const el = (
                <circle
                  ...
                  r={r}
                  fill="none"
                  stroke={seg.color || DEFAULT_COLORS[idx % DEFAULT_COLORS.length]}
                  strokeWidth="12"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-3">
            <span className="text-[10px] font-bold text-gray-500 uppercase">{centerLabel}</span>
              ...
              <div key={idx} className="flex items-center justify-between gap-2 text-xs">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    ...
                    style={{
                      backgroundColor: seg.color || DEFAULT_COLORS[idx % DEFAULT_COLORS.length]
                    }}
                  />
                  <span className="font-bold text-gray-700 truncate">{seg.label}</span>
                </div>
                <span className="font-black text-gray-900 shrink-0">
                  ${Number(seg.value).toLocaleString('es-MX')} · {pct}%
                </span>
              </div>
            );
          })}
