    ...
    <div className="space-y-3 mb-5">
      <div className="flex items-center justify-between px-1">
        <span className="text-xs font-bold text-gray-600 uppercase tracking-wider flex items-center gap-1.5">
          <Calendar className="w-3.5 h-3.5 text-[#E30613]" /> Selecciona tu Plazo Banorte:
        </span>
        <span className="text-[11px] text-gray-500 font-medium">Tasa de interés congelada</span>
      </div>
      ...
      {options.map((option) => {
        ...
        return (
          <div
            ...
            className={`cursor-pointer relative flex items-center justify-between p-4 sm:p-5 rounded-2xl border transition-all duration-200 ${
              isSelected
                ? 'bg-[#FFF0F1] border-[#E30613] shadow-sm ring-1 ring-[#E30613]/30'
                : 'bg-white border-gray-200 hover:border-gray-300 hover:bg-gray-50/70 shadow-xs'
            }`}
          >
            {/* Indicador Recomendado */}
            {option.recommended && (
              <span className="absolute -top-2.5 right-4 bg-[#E30613] text-white text-[10px] font-extrabold px-3 py-0.5 rounded-full flex items-center gap-1 shadow-xs uppercase tracking-wider">
                <Star className="w-2.5 h-2.5 fill-current" /> Mejor Opción Banorte
              </span>
            )}
            ...
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <div className="text-base sm:text-lg font-bold text-gray-900 flex items-center gap-2">
                  ...
                  {isSelected && (
                    <span className="text-[10px] bg-[#FFF0F1] text-[#E30613] px-2 py-0.5 rounded font-bold border border-[#F3C5C8]">
                      SELECCIONADO
                    </span>
                  )}
                </div>
                <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-1.5 font-medium">
                  ...
                  <span>CAT Promedio: <strong className="text-gray-900">{option.cat}%</strong> sin comisiones</span>
                </div>
              </div>
            </div>
