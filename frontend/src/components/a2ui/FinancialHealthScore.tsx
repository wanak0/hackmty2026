    ...
    <div className="space-y-4 mb-5">
      ...
      <div className="bg-white border border-amber-200 rounded-2xl p-5 sm:p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="flex items-center gap-1.5 text-xs text-gray-500 font-bold uppercase tracking-wider">
              ...
              <span>Indicador financiero de ejemplo</span>
            </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-200 shadow-xs">
          <div className="flex justify-between text-xs text-gray-700 mb-2 font-semibold">
            <span>Relación entre deuda e ingreso (DTI):</span>
            <span className="font-bold text-amber-700">{dti}%</span>
          </div>
          <div className="w-full bg-gray-100 h-2.5 rounded-full overflow-hidden p-0.5 border border-gray-200">
            <div
              className="bg-[#8F0017] h-full rounded-full transition-all duration-500"
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-3">
        <span className="text-xs font-bold text-gray-600 uppercase tracking-wider flex items-center gap-1.5">
          <AlertCircle className="w-4 h-4 text-amber-600" /> Plan de Acción
          Financiero Recomendado:
        </span>
        <div className="space-y-2.5">
          {recommendations.map((rec, idx) => (
            <div
              key={idx}
              className="text-xs text-gray-800 flex items-start gap-3 bg-[#F8F9FB] p-3.5 rounded-xl border border-gray-200 hover:border-gray-300 transition-colors"
            >
